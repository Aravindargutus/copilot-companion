...
- Commands reference scripts/targets that exist (verified by inspection)
...
2. Cross-reference with actual codebase:
   - Review documented commands by **inspection only** — do NOT execute them. A CLAUDE.md may document destructive or stateful commands (deploys, DB migrations, `rm`, release scripts), and an audit must never trigger side effects. Verify a command by checking that its script/target exists (e.g. in `package.json`, `Makefile`, or `scripts/`), not by running it.
