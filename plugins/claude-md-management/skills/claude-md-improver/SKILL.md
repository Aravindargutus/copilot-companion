...
Find all CLAUDE.md files in the repository (skipping `node_modules` and `.git` so vendored files don't crowd out the project's own):

```bash
find . -type d \( -name node_modules -o -name .git \) -prune -o -type f \( -name "CLAUDE.md" -o -name "CLAUDE.local.md" \) -print 2>/dev/null | head -50
```
